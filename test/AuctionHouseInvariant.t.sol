...
contract AuctionHouseHandler is Test {
    ...
    function submitOrder(uint256 traderSeed, uint8 sideRaw, uint8 flowRaw, int24 priceTick, uint128 qty) public {
    ...
    function finalizeBatch(uint256 timeDelta) public {
        ...
        if (currentBatch < newBatch && !finalizedBatches[currentBatch]) {
            try auctionHouse.finalizeStep(spotMarketId, currentBatch, 100) returns (
                AuctionHouse.FinalizePhase, bool done
            ) {
                if (done) {
...
contract AuctionHouseInvariantTest is Test {
    ...
    function invariant_ClearingTickInRange() public view {
        ...
        for (uint64 i = 0; i < 5 && i <= currentBatchId; i++) {
            ...
            try auctionHouse.getClearing(spotMarketId, batchId) returns (
                OrderTypes.Clearing memory bidClearing, OrderTypes.Clearing memory askClearing
            ) {
    ...
    function invariant_ConstantsImmutable() public view {
        ...
        assertEq(auctionHouse.MIN_TICK(), -887_272, "MIN_TICK changed");
        assertEq(auctionHouse.MAX_TICK(), 887_272, "MAX_TICK changed");
