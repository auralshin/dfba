...
contract AutoSettlementTest is Test {
    ...
    function setUp() public {
        ...
        // Fund test accounts
        baseToken.mint(alice, 100000 * 10**18);
        baseToken.mint(bob, 100000 * 10**18);
        baseToken.mint(carol, 100000 * 10**18);
        baseToken.mint(dave, 100000 * 10**18);
        quoteToken.mint(alice, 100000 * 10**18);
        quoteToken.mint(bob, 100000 * 10**18);
        quoteToken.mint(carol, 100000 * 10**18);
        quoteToken.mint(dave, 100000 * 10**18);
    }
    ...
    function testAutoSettlementMultipleBatches() public {
        ...
        vm.prank(alice);
        auctionHouse.submitOrder(OrderTypes.Order({
            trader: alice, marketId: marketId, side: OrderTypes.Side.Buy, flow: OrderTypes.Flow.Maker,
            priceTick: 100, qty: 1000, nonce: 0, expiry: uint64(block.timestamp + 3600)
        }));
        ...
        vm.prank(bob);
        auctionHouse.submitOrder(OrderTypes.Order({
            trader: bob, marketId: marketId, side: OrderTypes.Side.Sell, flow: OrderTypes.Flow.Taker,
            priceTick: 100, qty: 500, nonce: 0, expiry: uint64(block.timestamp + 3600)
        }));
        ...
        vm.prank(carol);
        auctionHouse.submitOrder(OrderTypes.Order({
            trader: carol, marketId: marketId, side: OrderTypes.Side.Buy, flow: OrderTypes.Flow.Maker,
            priceTick: 102, qty: 300, nonce: 0, expiry: uint64(block.timestamp + 3600)
        }));
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            assertEq(uint256(phase), uint256(AuctionHouse.FinalizePhase.Done), string(abi.encodePacked("Batch ", uint2str(i), " should be finalized")));
        }
    }
    ...
    function testAutoSettlementEmptyBatch() public {
        ...
        if (batch1 < batch2) {
            ...
            if (phase1 != AuctionHouse.FinalizePhase.NotStarted) {
                assertEq(uint256(phase1), uint256(AuctionHouse.FinalizePhase.Done), "Empty batch should be finalized if started");
            }
        }
    }
    ...
    function testAutoSettlementBidAndAskAuctions() public {
        ...
        vm.prank(carol);
        auctionHouse.submitOrder(OrderTypes.Order({
            trader: carol, marketId: marketId, side: OrderTypes.Side.Sell, flow: OrderTypes.Flow.Maker,
            priceTick: 110, qty: 800, nonce: 0, expiry: uint64(block.timestamp + 3600)
        }));
        
        vm.prank(dave);
        auctionHouse.submitOrder(OrderTypes.Order({
            trader: dave, marketId: marketId, side: OrderTypes.Side.Buy, flow: OrderTypes.Flow.Taker,
            priceTick: 110, qty: 400, nonce: 0, expiry: uint64(block.timestamp + 3600)
        }));
