...
contract PerpRouter is EIP712, AccessControl {
    ...
    function submitOrder(OrderTypes.Order memory order, address collateral) 
        external 
    ...
    function _submitOrderInternal(OrderTypes.Order memory order, address collateral)
        internal
        returns (bytes32 orderId, uint64 batchId)
    {
        // Verify market type
    ...
    function _isReduceOnly(OrderTypes.Order memory order, int256 currentPosition)
        internal
        pure
        returns (bool)
    {
        ...
        int256 orderDelta = order.side == OrderTypes.Side.Buy
            ? int256(uint256(order.qty))
            : -int256(uint256(order.qty));
    ...
    function updatePosition(
        ...
        int256 delta = side == OrderTypes.Side.Buy
    ...
    function releaseIM(bytes32 orderId) external onlyRole(SETTLEMENT_ROLE) {
        ...
        VAULT.releaseInitialMargin(
            orderId,
            reserve.user,
            reserve.subaccountId,
            reserve.collateral
        );
